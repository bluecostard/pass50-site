...
const PROFILE_ID='census-chocolat-show-officiel';
const TIKTOK_URL='https://www.tiktok.com/@chocolat.show.officiel';
let attempts=0;
...
function baseProfile(){
  return {
    ...
    region:'CI',
    country:'CI',
    category:'Comédie / Humour / Divertissement',
    ...
    photoStatus:'missing',
    photoSource:'',
    photoNote:'Photo à valider depuis le compte TikTok @chocolat.show.officiel.',
    photoPosition:'50% 50%',
    ...
    linkChecks:{
      TikTok:verifiedLink('Compte TikTok @chocolat.show.officiel confirmé — Chocolat show officiel, recensé le 23 août 2026.')
    },
    verifiedPass50:false,
    censusStatus:'Recensé confirmé',
    verificationPriority:'P0',
    knownAlias:'Chocolat show officiel / @chocolat.show.officiel',
    source:{publisher:'Signalement PASS50 — profil TikTok Chocolat show officiel',date:'2026-08-23',url:TIKTOK_URL},
    notes:'Comédien / humoriste. Compte TikTok officiel @chocolat.show.officiel. La bio publique indique le management @legrandbicongo — deux fiches distinctes. Date de naissance non confirmée.'
  };
}
...
function applyProfile(){
  ...
  let profile=db.profiles.find(item=>{
    ...
    return item&&(
      ...
      ||name==='chocolat show officiel'
      ||handle==='chocolat.show.officiel'
      ||tiktok.includes('@chocolat.show.officiel')
    );
  });
  let changed=false;
