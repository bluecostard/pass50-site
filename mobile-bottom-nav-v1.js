...
(() => {
    ...
    style.textContent = `
      ...
      @media(max-width:680px){
        ...
        .p50-bottom-nav{
          ...
          display:grid;grid-template-columns:minmax(82px,1fr) minmax(104px,1.18fr) minmax(82px,1fr);align-items:end;
          width:min(350px,calc(100vw - 28px));min-height:70px;padding:8px 10px;
        ...
        .p50-bottom-link{
          ...
          display:grid;align-content:center;justify-items:center;gap:4px;padding:6px 4px;border-radius:16px;
          font-size:9px;font-weight:950;letter-spacing:.08px;line-height:1.1;text-decoration:none;
        ...
        .p50-bottom-link-ranking{
          position:relative;z-index:2;min-height:78px;margin-top:-18px;padding:7px 8px 6px;
      ...
      @media(max-width:360px){
        .p50-bottom-nav{width:calc(100vw - 20px);grid-template-columns:minmax(76px,1fr) minmax(98px,1.15fr) minmax(76px,1fr);padding-inline:7px}
        .p50-bottom-link{font-size:8.5px}
      }
    `;
  ...
  function navHtml() {
    return `<nav class="p50-bottom-nav" aria-label="Menu principal mobile" data-contract="${CONTRACT}">
      ...
      <a class="p50-bottom-link p50-bottom-link-ranking ${isFeed ? '' : 'active'}" href="./" data-p50-tab="ranking" ${isFeed ? '' : 'aria-current="page"'}>${rankingIcon()}<span>Classement</span></a>
