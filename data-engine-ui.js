(function(){
  ...
  function deDrawMetricsDiagnostic(pane){
    ...
    pane.innerHTML=`<div class="de-observability-shell">
      ...
      ].map(([label,value])=>`<div><strong>${deEsc(value)}</strong><span>${deEsc(label)}</span></div>`).join('')}</div><div class="media-hint">Cadences demandées : P0 ${deEsc(metricAutomation.expectedCadences?.p0||'—')} (prochain passage théorique ${deEsc(metricAutomation.nextExpectedAt?.p0?deTime(metricAutomation.nextExpectedAt.p0):'—')}) · P1 ${deEsc(metricAutomation.expectedCadences?.p1||'—')} (${deEsc(metricAutomation.nextExpectedAt?.p1?deTime(metricAutomation.nextExpectedAt.p1):'—')}) · P2 ${deEsc(metricAutomation.expectedCadences?.p2||'—')} (${deEsc(metricAutomation.nextExpectedAt?.p2?deTime(metricAutomation.nextExpectedAt.p2):'—')}). Les passages GitHub Actions peuvent être décalés.</div>${failedJobs.length?`<div class="section-head"><div class="section-title">Tâches échouées de l’orchestrateur</div><span class="muted">${deObsNumber(failedJobs.length)} récente(s)</span></div><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Échec</th><th>Collecteur</th><th>Plateforme</th><th>Profil</th><th>Cadence</th><th>Tentatives</th><th>Erreur sécurisée</th><th>Job</th></tr></thead><tbody>${failedJobs.map(row=>`<tr><td>${deEsc(row.failedAt?deTime(row.failedAt):'—')}</td><td>${deEsc(row.collector||'—')}</td><td>${deEsc(row.platform||'—')}</td><td>${deEsc(row.scopeId||'Global')}</td><td>${deEsc(row.cadence||'—')}</td><td>${deObsNumber(row.attempts)} / ${deObsNumber(row.maxAttempts)}</td><td>${deEsc(row.message||'Erreur sans détail')}</td><td><code>${deEsc(row.jobUuid||'—')}</code></td></tr>`).join('')}</tbody></table></div>`:''}<div class="de-toolbar">${['p0','p1','p2'].map(c=>`<button class="btn de-orchestrator-action" data-action="preview" data-cadence="${c}">Prévisualiser ${c.toUpperCase()}</button>`).join('')}<button class="btn primary de-orchestrator-action" data-action="enqueue" data-cadence="p0" ${schemaApplied?'':'disabled'}>Planifier un cycle</button><button class="btn de-orchestrator-action" data-action="work_one" ${schemaApplied?'':'disabled'}>Traiter une tâche</button><button class="btn de-orchestrator-action" data-action="recover_stale" ${schemaApplied?'':'disabled'}>Récupérer les tâches bloquées</button></div></section>
