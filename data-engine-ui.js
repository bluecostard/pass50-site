(function(){
  ...
  async function deOpenSocial(profileId){
    ...
    try{
      ...
      pane.innerHTML=`<div class="de-profile-head"><div><div class="section-title">Réseaux officiels · ${deEsc(data.profile.public_name)}</div><div class="muted">Un lien n’est public qu’après confirmation et avec une confiance ≥ ${data.threshold||90} %.</div></div><button class="btn" data-admin-tab="hub">Retour au Data Hub</button></div><div class="de-links">${DE.platforms.map(platform=>deLinkCard(profileId,platform,current[platform])).join('')}</div>`;
    }catch(err){pane.innerHTML=`<div class="de-error">${deEsc(err.message)}</div><button class="btn" data-admin-tab="hub">Retour</button>`;}
  }
  ...
  function deLinkCard(profileId,platform,link){
    ...
    return `<article class="de-link-card"><div class="de-link-head"><strong>${deEsc(platform)}</strong>${deStatus(status,confidence)}</div><form class="de-link-form" data-profile="${deEsc(profileId)}" data-platform="${deEsc(platform)}"><input type="url" name="url" value="${deEsc(url)}" placeholder="https://…" required><button class="btn primary" type="submit">Enregistrer</button><label class="de-confirm"><input type="checkbox" name="confirmedOfficial" required> Je confirme que ce lien correspond au compte officiel de cet influenceur.</label></form>${url?`<div class="de-link-meta">${deEsc(url)}${link?.checked_at?` · contrôlé ${deTime(link.checked_at)}`:''}</div><div class="de-link-actions"><a class="btn small" href="${deEsc(url)}" target="_blank" rel="noopener">Ouvrir ↗</a><button class="btn small danger de-reject-link" data-profile="${deEsc(profileId)}" data-platform="${deEsc(platform)}">Rejeter</button></div>`:''}</article>`;
  }
  ...
  document.addEventListener('submit',async e=>{
    if(e.target.id==='deBirthForm'){
      ...
      try{await deAction(button,async()=>{await apiFetch('facts.php',{method:'POST',body:{profileId:form.dataset.profile,factKey:'birth_date',value:String(fd.get('value')||''),sourceName:String(fd.get('sourceName')||''),sourceUrl:String(fd.get('sourceUrl')||'')}});await loadCloudState();render();await deOpenBirth(form.dataset.profile);toast('Source de naissance ajoutée');},'Vérification…');}catch(err){console.error(err);toast(err.message||'Source refusée');}
      return;
    }
