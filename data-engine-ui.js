(function(){
  ...
  function deRankingCurrentHtml(){
    ...
    const tableRows=rows.map(row=>{const publicRank=dePublicRank(row.profileId,DE.rankingLabPeriod),gap=row.rank&&publicRank?publicRank-row.rank:null,evolution=!row.classable?'Non classé':row.rankDelta===null?'Nouvelle entrée':row.rankDelta>0?`+${row.rankDelta}`:String(row.rankDelta),status=row.classable?'Classable':(row.exclusionReasons||[]).map(reason=>DE_RANKING_REASON_LABELS[reason]||reason).join(' · ');
