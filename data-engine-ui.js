(function(){
  ...
  function deDrawMetricsDiagnostic(pane){
    ...
    const ranking=data.ranking||{},volumes=data.volumes||{},fresh=data.freshness||{},collections=data.collections||{},canonical=data.canonicalSchema||{},metricCollectors=data.collectors||{},platforms=data.platforms||[],errors=collections.recentErrors||[];
    const kpis=[
    ...
    pane.innerHTML=`<div class="de-observability-shell">
      ...
      <section class="de-observability-card"><div class="section-head"><div><div class="section-title">Schéma canonique</div><span class="muted">Version ${deObsNumber(canonical.schemaVersion)} · ${deEsc(canonical.migrationStatus||'non installé')}</span></div>${canonical.migrationStatus==='applied'?'<span class="de-status verified">Installé</span>':'<span class="de-status candidate">À migrer</span>'}</div><div class="de-reason-grid">${[['Comptes',canonical.accounts],['Contenus',canonical.contents],['Captures',canonical.captures],['Jobs',canonical.jobs],['Runs',canonical.runs],['Quarantaines',canonical.quarantinedCaptures]].map(([label,value])=>`<div><strong>${value===null||value===undefined?'—':deObsNumber(value)}</strong><span>${deEsc(label)}</span></div>`).join('')}</div><div class="muted">Dernier backfill : ${deEsc(canonical.lastBackfillAt?deTime(canonical.lastBackfillAt):'Jamais')}</div></section>
      <section class="de-observability-card"><div class="section-head"><div><div class="section-title">COLLECTE YOUTUBE & X</div><div class="muted">Collecte expérimentale : les données ne modifient pas encore le classement public.</div></div><div><button class="btn de-collect-metrics" data-platform="YouTube">Collecter YouTube</button> <button class="btn de-collect-metrics" data-platform="X">Collecter X</button></div></div><label>Profil ciblé <select id="deMetricProfile"><option value="">Lot de profils vérifiés</option>${(db?.profiles||[]).filter(p=>p?.id).map(p=>`<option value="${deEsc(p.id)}">${deEsc(p.name||p.id)}</option>`).join('')}</select></label><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Plateforme</th><th>Configuration</th><th>Contenu détecté</th><th>Métrique capturée</th><th>Métrique indisponible</th><th>Dernière capture</th><th>Quota / blocage</th></tr></thead><tbody>${[['YouTube',metricCollectors.youtube||{}],['X',metricCollectors.x||{}]].map(([name,row])=>`<tr><td><strong>${name}</strong></td><td>${row.configured?'<span class="de-status verified">Configuré</span>':'<span class="de-status candidate">Non configuré</span>'}</td><td>${deObsNumber(row.contents)}</td><td>${deObsNumber(row.captures)} <span class="muted">(${deObsNumber(row.captures24h)} sur 24 h)</span></td><td>${deObsNumber(row.unavailableProfiles)}</td><td>${row.latestCaptureAt?deTime(row.latestCaptureAt):'Jamais'}</td><td>${Number(row.rateLimitedCount||0)?`<span class="de-status conflict">${deObsNumber(row.rateLimitedCount)} quota(s)</span>`:(row.lastStatus==='unavailable_or_blocked'?'<span class="de-status candidate">Bloqué ou indisponible</span>':'Aucun')}${row.lastError?` · ${deEsc(row.lastError)}`:''}</td></tr>`).join('')}</tbody></table></div></section>
