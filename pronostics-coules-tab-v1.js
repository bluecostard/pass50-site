...
function installEmbedMode(){
  ...
  const sendHeight=()=>{
    try{
      ...
      parent?.postMessage?.({type:'pass50-coules-height',height:h},'*');
...
function installPronoTabs(){
  ...
  function setMode(mode,push=true){
    ...
    if(coules)setTimeout(()=>document.getElementById('p50CoulesPronoFrame')?.contentWindow?.postMessage({type:'pass50-coules-refresh'},'*'),80);
  }
