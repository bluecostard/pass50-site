...
const PROFILE_ID='census-lise-akrassi';
const TIKTOK_URL='https://www.tiktok.com/@lise.akrassi.offi';
let attempts=0;
...
function baseProfile(){
  return {
    id:PROFILE_ID,
    name:'Lise Akrassi',
    handle:'@lise.akrassi.offi',
    initials:'LA',
    region:'CI',
    category:'Lifestyle / Business / Foi',
    platforms:['TikTok'],
    ...
    photoStatus:'missing',
    photoSource:'',
    photoNote:'Photo à valider depuis le compte TikTok @lise.akrassi.offi.',
    photoPosition:'50% 50%',
    ...
    linkChecks:{
      TikTok:verifiedLink('Compte TikTok @lise.akrassi.offi confirmé — recensé le 19 août 2026.')
    },
    verifiedPass50:false,
    censusStatus:'Recensé confirmé',
    verificationPriority:'P1',
    source:{publisher:'Signalement PASS50 — profil TikTok Lise Akrassi',date:'2026-08-19',url:TIKTOK_URL},
    notes:'Lise Akrassi official (@lise.akrassi.offi). Faith & Business, fondatrice PINK AGENCY. Profil recensé et relié au Radar LIVE PASS50.'
  };
}
...
function applyProfile(){
  ...
  let profile=db.profiles.find(item=>{
    ...
    return item&&(item.id===PROFILE_ID||name.includes('lise akrassi')||handle==='@lise.akrassi.offi'||handle==='lise.akrassi.offi');
  });
  let changed=false;
