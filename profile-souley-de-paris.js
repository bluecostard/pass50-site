...
const PROFILE_ID='census-souley-de-paris';
const TIKTOK_URL='https://www.tiktok.com/@souleydeparis';
let attempts=0;

function verifiedLink(message){
  return {status:'manual_verified',checkedAt:'2026-08-24T16:00:00.000Z',message};
}
...
function baseProfile(){
  return {
    ...
    region:'DIASPORA',
    country:'DIASPORA',
    category:'TikTok / Live / Actualité',
    occupation:'Streamer TikTok',
    platforms:['TikTok'],
    ...
    photoStatus:'missing',
    photoSource:'',
    photoNote:'Photo à valider depuis le compte TikTok @souleydeparis.',
    photoPosition:'50% 50%',
    ...
    linkChecks:{
      TikTok:verifiedLink('Compte TikTok @souleydeparis confirmé — Souley de Paris, recensé le 24 août 2026.')
    },
    verifiedPass50:false,
    censusStatus:'Recensé confirmé',
    verificationPriority:'P0',
    knownAlias:'Souley de Paris / SOULEY DE-PARIS / B-52 / @souleydeparis',
    source:{publisher:'Signalement PASS50 — profil TikTok Souley de Paris',date:'2026-08-24',url:TIKTOK_URL},
    notes:'Streamer TikTok. Compte officiel @souleydeparis. Identité civile et date de naissance non confirmées.'
  };
}
...
function applyProfile(){
  ...
  let profile=db.profiles.find(item=>{
    ...
    return item&&(
      item.id===PROFILE_ID
      ||name==='souley de paris'
      ||compact==='souleydeparis'
      ||handle==='souleydeparis'
      ||tiktok.includes('@souleydeparis')
    );
  });
  let changed=false;
