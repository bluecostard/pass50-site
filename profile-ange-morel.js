...
const PROFILE_ID='census-ange-morel';
const TIKTOK_URL='https://www.tiktok.com/@angemorel4';
const TIKTOK_LIVE_URL='https://www.tiktok.com/@angemorel4/live';
let attempts=0;

function verifiedLink(message){
  return {status:'manual_verified',checkedAt:'2026-08-19T23:45:00.000Z',message};
}

function baseProfile(){
  return {
    id:PROFILE_ID,
    name:'Ange-Morel Your Eyes',
    handle:'@angemorel4',
    initials:'AM',
    region:'BOTH',
    category:'Lifestyle / Divertissement / TikTok Lives',
    platforms:['TikTok'],
    ...
    photoStatus:'missing',
    photoSource:'',
    photoNote:'Photo à valider depuis le compte TikTok @angemorel4.',
    photoPosition:'50% 50%',
    ...
    linkChecks:{
      TikTok:verifiedLink('Compte TikTok @angemorel4 confirmé — créateur (2,3 M abonnés), signalé en direct le 19 août 2026.')
    },
    verifiedPass50:false,
    censusStatus:'Recensé confirmé',
    verificationPriority:'P0',
    source:{publisher:'Signalement PASS50 — profil TikTok Ange-Morel Your Eyes',date:'2026-08-19',url:TIKTOK_URL},
    notes:'Créateur connu sous le nom Ange-Morel Your Eyes (@angemorel4). Lifestyle, divertissement et lives TikTok. Compte relié au Radar LIVE ; direct signalé manuellement le 19 août 2026.'
  };
}
...
function ensureManualLive(){
  ...
  db.liveStreams.push({
    id:'live_angemorel_'+Date.now(),
    profileId:PROFILE_ID,
    platform:'TikTok',
    url:TIKTOK_LIVE_URL,
    title:'Direct en cours — Ange-Morel Your Eyes',
    status:'live',
    source:'manual',
...
function applyProfile(){
  ...
  let profile=db.profiles.find(item=>{
    ...
    return item&&(item.id===PROFILE_ID||name.includes('ange-morel')||name.includes('angemorel')||handle==='@angemorel4'||handle==='angemorel4');
  });
  let changed=false;
