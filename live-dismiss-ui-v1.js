...
function findLiveByUrl(value){const wanted=normalizeUrl(value);return currentLives().find(item=>normalizeUrl(item?.url)===wanted&&item?.status==='live')||null}
...
function enhanceAdmin(){
  document.querySelectorAll('#adminPane .signal').forEach(card=>{
    ...
    const live=findLiveByUrl(open.href);if(!live||live.source==='manual')return;
...
async function dismiss(button){
  ...
  if(!confirm('Supprimer ce faux direct du radar ?'))return;
  ...
  try{
    ...
    if(Array.isArray(db?.liveStreams))db.liveStreams=db.liveStreams.filter(item=>!(String(item.profileId)===profileId&&String(item.platform)===platform&&normalizeUrl(item.url)===normalizeUrl(url)));
