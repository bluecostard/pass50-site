...
function p50ym_safe_connections(PDO $pdo): array {
    $empty=['schemaReady'=>false,'connections'=>[],'summary'=>['total'=>0,'active'=>0,'mapped'=>0,'unmapped'=>0,'reauthorizationRequired'=>0]];
    if(!p50_metrics_table_exists($pdo,'p50_youtube_oauth_connections'))return $empty;
    try{
        ...
        return array_merge($empty,['error'=>'youtube_oauth_unavailable']);
    }
