...
$trendStmt=$pdo->prepare("SELECT t.rank_position,t.previous_rank,t.rank_delta,t.score,t.confidence,t.view_delta,t.interaction_delta,
  ...
  c.id content_id,c.profile_id,c.platform,c.content_type,c.canonical_url,c.title,c.published_at,c.platform_content_id,c.metadata_json,
...
foreach($trendStmt->fetchAll() as $row){
    ...
    $trends[]=[
        ...
        'title'=>(string)$row['title'],'url'=>(string)$row['canonical_url'],'thumbnailUrl'=>$thumbnail,
        'publishedAt'=>$row['published_at']?gmdate('c',strtotime((string)$row['published_at'].' UTC')):null,
...
if($profileId!==''){
    ...
    foreach($newsStmt->fetchAll() as $row)$news[]=[
        'id'=>(int)$row['id'],'contentId'=>$row['content_id']===null?null:(int)$row['content_id'],'platform'=>(string)$row['platform'],
        'itemType'=>(string)$row['item_type'],'url'=>(string)$row['canonical_url'],'title'=>(string)$row['title'],
        'thumbnailUrl'=>(string)($row['thumbnail_url']??''),'publishedAt'=>$row['source_published_at']?gmdate('c',strtotime((string)$row['source_published_at'].' UTC')):null,
...
json_response([
    ...
    'rules'=>['maxPerProfile'=>2,'topLimit'=>5,'officialContentAutomatic'=>true,'externalNewsHumanValidation'=>true],
