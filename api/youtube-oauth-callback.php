...
if ($state === '') p50yo_redirect_result('error', 'missing_state');
...
try {
    ...
    if (!$oauthState || $oauthState['consumed_at'] !== null || strtotime((string)$oauthState['expires_at']) < time()) {
        throw new RuntimeException('État OAuth invalide ou expiré.');
    }
    $userId = (string)$oauthState['user_id'];
    $db->prepare('UPDATE p50_youtube_oauth_states SET consumed_at=NOW() WHERE state_hash=?')->execute([$stateHash]);
    $db->commit();
} catch (Throwable $e) {
...
try {
    ...
    $sql = "INSERT INTO p50_youtube_oauth_connections
        ...
        ON DUPLICATE KEY UPDATE
          ...
          access_expires_at=VALUES(access_expires_at),status='active',last_error=NULL,connected_at=NOW(),last_refreshed_at=NOW()";
    ...
    db()->prepare('DELETE FROM p50_youtube_oauth_states WHERE user_id=? OR expires_at<NOW()')->execute([$userId]);
    ...
    if ($userId !== '') {
        $existing = p50yo_connection_for_user($userId);
        if ($existing) {
            db()->prepare("UPDATE p50_youtube_oauth_connections SET status='error',last_error=? WHERE user_id=?")
                ->execute([mb_substr($e->getMessage(), 0, 1000), $userId]);
        }
    }
