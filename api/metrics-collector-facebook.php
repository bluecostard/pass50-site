...
function p50_mc_facebook(PDO $pdo,array $official,int $limit,string $observedAt,string $runUuid,callable $fetch,array &$result): void {
    ...
    $headers=['Authorization: Bearer '.$credentials['secret']];$pageId=(string)$credentials['pageId'];
    if($pageId===''&&$kind==='id')$pageId=$identity;
    if($pageId===''){
        $resolve=p50_mc_request($fetch,p50_msc_query_url('https://graph.facebook.com/v22.0/'.rawurlencode($identity),['fields'=>'id']),$headers,$result);
    ...
    $pageResponse=p50_mc_request($fetch,p50_msc_query_url('https://graph.facebook.com/v22.0/'.rawurlencode($pageId),['fields'=>'id,name,username,followers_count,fan_count']),$headers,$result);
    ...
    $fields='id,message,created_time,permalink_url,status_type,attachments{media_type,type},comments.limit(0).summary(true),shares,like_reactions:reactions.type(LIKE).limit(0).summary(true),reactions.limit(0).summary(true)';
    $after=null;$collected=0;
    do{
        $query=['fields'=>$fields,'limit'=>min(100,$limit-$collected)];if($after!==null)$query['after']=$after;
        $postsResponse=p50_mc_request($fetch,p50_msc_query_url('https://graph.facebook.com/v22.0/'.rawurlencode($pageId).'/posts',$query),$headers,$result);
        ...
        foreach((array)($payload['data']??[]) as $post){
            if($collected++>=$limit)break;$post=(array)$post;$insights=[];
            $insightResponse=p50_mc_request($fetch,p50_msc_query_url('https://graph.facebook.com/v22.0/'.rawurlencode((string)$post['id']).'/insights',['metric'=>'post_impressions_unique,post_clicks,post_video_views']),$headers,$result);
            ...
            $item=['id'=>$post['id']??'','url'=>$post['permalink_url']??'','type'=>p50_mc_facebook_type($post),'title'=>$post['message']??'','publishedAt'=>$post['created_time']??null,'metadata'=>['statusType'=>$post['status_type']??null]];
            p50_msc_store_content($pdo,$official,$account,'Facebook',$item,'facebook_graph_api','Page posts+insights',$credentials['mode'],$observedAt,$runUuid,$result,
              ['views'=>p50_mc_int($insights,array_key_exists('post_video_views',$insights)?'post_video_views':'post_impressions_unique'),'likes'=>p50_mc_int(['value'=>$likes],'value'),'comments'=>p50_mc_int(['value'=>$comments],'value'),'shares'=>p50_mc_int(['value'=>$shares],'value')],
              ['reactionsTotal'=>p50_mc_int(['value'=>$reactions],'value'),'likeReactions'=>p50_mc_int(['value'=>$likes],'value'),'videoViews'=>p50_mc_int($insights,'post_video_views'),'reach'=>p50_mc_int($insights,'post_impressions_unique'),'postClicks'=>p50_mc_int($insights,'post_clicks')],(int)$postsResponse['status'],hash('sha256',(string)$postsResponse['body'].'|'.(string)$insightResponse['body']));
        }
