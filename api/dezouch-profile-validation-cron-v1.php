...
const P50_DEZOUCH_VALIDATION_VERSION='DEZOUCH-PROFILE-VALIDATION-V1.0';
...
$links=[
    'Instagram'=>'https://www.instagram.com/dezouch_officiel/',
    'TikTok'=>'https://www.tiktok.com/@dezouch_officiel',
    'Facebook'=>'https://www.facebook.com/dezouch.officiel',
];
...
try{
    ...
    foreach((array)($state['profiles']??[]) as $index=>$candidate){
        $id=strtolower(trim((string)($candidate['id']??'')));$name=strtolower(trim((string)($candidate['name']??'')));
        if($id==='census-dezouch-officiel'||$name==='dezouch officiel'){$profileIndex=(int)$index;break;}
    }
    if($profileIndex<0)throw new RuntimeException('Fiche Dezouch officiel introuvable.');
    $profile=&$state['profiles'][$profileIndex];$profileId=(string)$profile['id'];
    ...
    $profile['platforms']=is_array($profile['platforms']??null)?$profile['platforms']:[];$validated=[];
    foreach($links as $platform=>$url){
        $normalized=p50_de_normalize_social_url($platform,$url);
        if($normalized===''||!p50_platform_host_ok($platform,$normalized)||!p50_de_direct_social_path($platform,$normalized))throw new RuntimeException('Lien Dezouch invalide pour '.$platform.'.');
        ...
        p50_de_log_social_action($profileId,$platform,'confirm',(string)($profile['links'][$platform]??''),$normalized,['id'=>'owner-pass50','role'=>'owner','display_name'=>'Propriétaire PASS50'],['confirmed'=>true,'permanentValidation'=>true,'version'=>P50_DEZOUCH_VALIDATION_VERSION]);
        $profile['links'][$platform]=$normalized;
        $profile['linkChecks'][$platform]=['status'=>'owner_verified','checkedAt'=>gmdate(DATE_ATOM),'message'=>'Compte officiel protégé après validation du propriétaire PASS50','persistedServerSide'=>true,'protectedBy'=>'PASS50-STATE-LINK-PROTECTION-V4.1'];
        $profile['platforms'][]=$platform;$validated[$platform]=$normalized;
    }
