...
foreach(['token','secret','url','accountUrl','contentUrl','endpoint','headers','authorization','cookie'] as $forbidden)if(array_key_exists($forbidden,$input))json_response(['error'=>'Paramètre interdit.'],422);
$platform=p50_mc_platform((string)($input['platform']??''));if($platform==='')json_response(['error'=>'Plateforme non autorisée.'],422);
$profileId=trim((string)($input['profileId']??''));
if(!in_array($action,['collect_profile','collect_batch'],true))json_response(['error'=>'Action inconnue.'],422);
if($action==='collect_profile'&&$profileId==='')json_response(['error'=>'profileId obligatoire.'],422);
$lock='pass50_metrics_collect_'.strtolower($platform).'_'.($action==='collect_profile'?hash('sha256',$profileId):'batch');
...
try{
    if($action==='collect_profile'){
        ...
        $response=['ok'=>true,'mode'=>'collect_profile','summary'=>p50_mc_summary([$result])['summary'],'details'=>[$result],'experimental'=>true];
    }else{
        ...
        $response=['ok'=>true,'mode'=>'collect_batch']+$result+['experimental'=>true];
    }
}catch(Throwable $error){
    error_log('PASS50 canonical metrics collection: '.p50_metrics_safe_error($error->getMessage()));
    $response=['error'=>'Collecte métrique interrompue.','detail'=>p50_metrics_safe_error($error->getMessage())];$httpStatus=500;
