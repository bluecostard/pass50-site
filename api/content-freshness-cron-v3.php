...
if($action==='probe')json_response(['ok'=>true,'action'=>'probe','dispatchId'=>$dispatchId,'version'=>P50_CONTENT_FRESHNESS_V3_VERSION,'bucketSeconds'=>P50_CONTENT_FRESHNESS_V3_BUCKET_SECONDS,'publicStateWrites'=>0]);
...
function p50_cf3_authorized_rows(PDO $pdo,array $ranked): array {
    ...
    $summary=['verified'=>0,'enabled'=>0,'authorized'=>0,'disabled'=>0,'configurationMissing'=>0,'authorizationRequired'=>0,'unsupported'=>0];$byPlatform=[];
    ...
    $authorized=[];
    foreach($rows as $row){
        $profileId=(string)$row['profile_id'];$platform=p50_mc_platform((string)$row['platform']);p50_cf3_platform_counter($byPlatform,$platform,'verified');
        if(!p50_mc_platform_enabled($platform)){$summary['disabled']++;p50_cf3_platform_counter($byPlatform,$platform,'disabled');continue;}
        $summary['enabled']++;p50_cf3_platform_counter($byPlatform,$platform,'enabled');$access=p50_mc_public_access($platform,$profileId);$mode=(string)($access['mode']??'');
        ...
        $summary['authorized']++;p50_cf3_platform_counter($byPlatform,$platform,'authorized');$authorized[]=['profileId'=>$profileId,'platform'=>$platform,'lastContentAt'=>$seen[$profileId.'|'.$platform]??null];
    }
...
function p50_cf3_select(array $ranked,array $authorized,int $profileLimit=P50_CONTENT_FRESHNESS_V3_PROFILE_LIMIT,int $jobLimit=P50_CONTENT_FRESHNESS_V3_JOB_LIMIT): array {
    ...
    $platforms=[];foreach($selectedJobs as $row)p50_cf3_platform_counter($platforms,(string)$row['platform'],'selected');ksort($platforms,SORT_NATURAL|SORT_FLAG_CASE);
...
function p50_cf3_enqueue(PDO $pdo,array $row,string $dispatchId,int $now): array {
    $profileId=(string)$row['profileId'];$platform=(string)$row['platform'];$start=(int)(floor($now/P50_CONTENT_FRESHNESS_V3_BUCKET_SECONDS)*P50_CONTENT_FRESHNESS_V3_BUCKET_SECONDS);$bucket=gmdate('YmdHis',$start);$observedAt=gmdate('Y-m-d H:i:s',$now);
    $idempotency=hash('sha256',implode('|',[P50_CONTENT_FRESHNESS_V3_VERSION,$bucket,$profileId,$platform]));
    $payload=['profileId'=>$profileId,'platform'=>$platform,'contentLimit'=>4,'observedAt'=>$observedAt,'liveConfirmed'=>false,'cadence'=>'p0','bucket'=>$bucket,'dispatchId'=>$dispatchId,'reason'=>'content_freshness_v3'];
...
try{
    ...
    for($iteration=1;$iteration<=24;$iteration++){
        ...
        if((int)($jobRow['priority']??-1)===5){$platform=p50_mc_platform((string)($jobRow['platform']??'Unknown'));p50_cf3_platform_counter($processedByPlatform,$platform,'processed');p50_cf3_platform_counter($processedByPlatform,$platform,(string)($work['status']??'unknown'));}
    }
