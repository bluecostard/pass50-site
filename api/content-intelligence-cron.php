...
try{
    $result=p50_ci_refresh(db());
    $result['dispatchId']=$dispatchId;
    $result['durationMs']=(int)round((microtime(true)-$started)*1000);
    json_response($result);
}catch(Throwable $error){
    error_log('PASS50 content intelligence cron: '.substr($error->getMessage(),0,300));
    json_response(['error'=>'Actualisation des contenus interrompue.','dispatchId'=>$dispatchId,'publicStateWrites'=>0],500);
}
