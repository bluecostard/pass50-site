...
const P50_METRICS_PUBLIC_BASELINE_VERSION='PUBLIC-BASELINE-P1-V1.0';
...
function p50_mopb_select(PDO $pdo,string $dispatchId,?string $now=null): array {
    ...
    $ids=p50_mopb_public_profile_ids($pdo,$cfg['p1Max']);
    $rows=p50_mopb_verified_rows($pdo,$ids);$selectionTime=strtotime($now??'now');if($selectionTime===false)$selectionTime=time();
    $summary=['publicProfiles'=>count($ids),'eligibleProfiles'=>count(array_unique(array_column($rows,'profile_id'))),'eligibleLinks'=>count($rows),'jobsCreated'=>0,'duplicateJobs'=>0,'skippedFresh'=>0,'skippedConfiguration'=>0,'skippedAuthRequired'=>0,'skippedUnsupported'=>0];
    $candidates=[];
    foreach($rows as $row){
        $profileId=(string)$row['profile_id'];$platform=(string)$row['platform'];$access=p50_mc_public_access($platform,$profileId);
        if(!$access['configured']){$summary['skippedConfiguration']++;continue;}
        if(!$access['authorized']){$summary['skippedAuthRequired']++;continue;}
        ...
        if($last&&strtotime((string)$last)>=$selectionTime-$cfg['fresh']['p1']*60){$summary['skippedFresh']++;continue;}
        ...
        $higher->execute([$profileId,$platform,$cadence['priority']]);if((int)$higher->fetchColumn()>0){$summary['skippedFresh']++;continue;}
