...
foreach ($items as $platform => $urlValue) {
    ...
    $hostOk = p50_platform_host_ok((string)$platform,$r['finalUrl'] ?: $url);
    $isSearch = (bool)preg_match('#/(search|results|explore/search)#i', (string)(parse_url($r['finalUrl'] ?: $url,PHP_URL_PATH) ?: '')) || str_contains((string)(parse_url($r['finalUrl'] ?: $url,PHP_URL_QUERY) ?: ''),'search_query=');
    $status = !$hostOk ? 'wrong_platform' : ($isSearch ? 'search_not_official' : ($r['ok'] ? 'ok' : (in_array($r['status'],[403,429],true) ? 'blocked_but_exists' : 'broken')));
    $out[$platform] = [
        'status'=>$status,
        'httpStatus'=>$r['status'],
        'url'=>$url,
