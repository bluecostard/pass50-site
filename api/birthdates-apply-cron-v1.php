...
$updates=['eunice'=>['birthDate'=>'1995-05-21','birthYear'=>1995,'sources'=>[
...
try{
 ...
 foreach($state['profiles'] as &$profile){$id=(string)($profile['id']??'');if(!isset($updates[$id]))continue;$u=$updates[$id];if(trim((string)($profile['birthDate']??''))!=='')continue;
  ...
  $profile['birthManualLocked']=false;$profile['birthEvidence']=['status'=>'confirmed','sources'=>$u['sources'],'checkedAt'=>gmdate('c')];
