...
const PASS50_V9={photoCandidates:[],photoProfileId:null,preview:null,previewEventId:null,news:[],newsProfileId:null,newsDays:15};
...
(function(){
  ...
  async function p50ValidateTriggerForm(form){
    ...
    try{
      const btn=form.querySelector('button[type=submit]');const old=btn.textContent;btn.disabled=true;btn.textContent='ANALYSE…';
      const preview=await apiFetch('content-preview.php',{method:'POST',body:{url}});
      let ev=primaryEvent(id);const patch={type:String(fd.get('type')||'Vidéo'),title:String(fd.get('title')||preview.title||`Buzz de ${p.name}`),platforms:[preview.platform||'Web'],metric:'Lien original validé',publishedLabel:'Validation manuelle',reason:String(fd.get('reason')||'Contenu original validé par le propriétaire PASS50.'),url:preview.canonicalUrl||url,icon:preview.platform==='YouTube'||preview.platform==='TikTok'||preview.platform==='Instagram'||preview.platform==='Facebook'?'▶':'📰',confidence:'élevée',originalLinkValidated:true,originalLinkValidatedAt:new Date().toISOString(),coverCandidateUrl:preview.thumbnail||ev?.coverCandidateUrl||'',coverStatus:preview.thumbnail?'pending':(ev?.coverStatus||'missing'),coverSource:preview.source||ev?.coverSource||'',coverNote:preview.thumbnail?'Couverture détectée depuis le lien original, à valider.':(ev?.coverNote||'')};if(ev)Object.assign(ev,patch);else db.events.push({id:'trigger_'+id+'_'+Date.now(),profileId:id,...patch});save();render();p50v9RenderNews();toast('Lien original validé dans la FI');btn.disabled=false;btn.textContent=old;
  ...
  p50v9SaveLinks=async function(id,card){
    ...
    card.querySelectorAll('[data-link-platform]').forEach(input=>{
      ...
      if(url){p.links[platform]=url;p.platforms=[...new Set([...(p.platforms||[]),platform])];p.linkChecks[platform]={status:'pending',checkedAt:null};
        if(confirmed&&p50v9IsDirectPlatformLink(platform,url))tasks.push(apiFetch('social-links.php',{method:'POST',body:{action:'save',profileId:id,platform,url,confirmedOfficial:true}}).then(()=>{p.linkChecks[platform]={status:'ok',checkedAt:new Date().toISOString(),message:'Validé manuellement par le propriétaire'}}).catch(err=>{p.linkChecks[platform]={status:'broken',checkedAt:new Date().toISOString(),message:err.message||'Validation serveur impossible'}}));
      }else{delete p.links[platform];p.linkChecks[platform]={status:'search_not_official',checkedAt:null};}
    });
    save();if(tasks.length)await Promise.allSettled(tasks);save();render();toast(confirmed?'Liens officiels enregistrés':'Liens enregistrés localement · confirmation requise pour publication');
  };
