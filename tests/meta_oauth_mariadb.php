...
require dirname(__DIR__).'/api/meta-oauth-assets.php';
...
$pdo->prepare("INSERT INTO p50_meta_oauth_assets(user_id,platform,asset_id,profile_id,asset_name,username,parent_page_id,access_token_encrypted,status) VALUES(?,?,?,?,?,?,?,?,'active')")->execute(['u1','Instagram','ig1','profile1','Compte IG','compte_ig','page1',p50mo_encrypt('page-token')]);

p50mo_replace_assets_for_user('u1',[[
    'platform'=>'Instagram','asset_id'=>'ig1','profile_id'=>null,'name'=>'Compte IG','username'=>'compte_ig',
    'url'=>'https://www.instagram.com/compte_ig/','picture'=>'','parent'=>'page1','token'=>'new-page-token','tasks'=>null,
]]);
$mapped=(string)$pdo->query("SELECT profile_id FROM p50_meta_oauth_assets WHERE user_id='u1' AND asset_id='ig1'")->fetchColumn();
must($mapped==='profile1','L’association manuelle doit survivre à la redécouverte Meta.');
...
echo json_encode(['ok'=>true,'schemaIdempotent'=>true,'encryptedAssetToken'=>true,'manualMappingPreserved'=>true,'deletionCompleted'=>true],JSON_UNESCAPED_SLASHES).PHP_EOL;
