...
$pdo->prepare("UPDATE p50_metric_jobs SET status='failed',attempts=3,lock_token='fixture-lock',last_error=? WHERE id=?")->execute([$sensitiveError,$failedFixture['id']]);
...
$failure=$failures[0];
orchestrator_must($failure['platform']==='YouTube'&&$failure['scopeId']==='live'&&$failure['cadence']==='p0','Plateforme, profil et cadence du job failed');
orchestrator_must($failure['attempts']===3&&$failure['maxAttempts']===4,'Tentatives du job failed');
...
foreach(['payload_json','lock_token','idempotency_key'] as $forbidden)orchestrator_must(!array_key_exists($forbidden,$failure),'Champ sensible absent du diagnostic : '.$forbidden);
