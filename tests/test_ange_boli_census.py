...
OVERLAY = (ROOT / "profile-ange-boli.js").read_text(encoding="utf-8")

PROFILE_ID = "census-ange-boli"
NAME = "Ange Boli"
HANDLE = "@angeboli7"
TIKTOK = "https://www.tiktok.com/@angeboli7"
OTHER_IDS = ("census-ange-arthur", "census-ange-freddy")
...
class AngeBoliCensusTests(unittest.TestCase):
    def census_row(self):
        matches = [item for item in CENSUS if item.get("id") == PROFILE_ID]
        self.assertEqual(len(matches), 1)
        return matches[0]

    def test_unique_handle_and_not_other_ange(self):
        ...
        ids = {item.get("id") for item in CENSUS}
        for other in OTHER_IDS:
            self.assertIn(other, ids)
            self.assertNotEqual(other, PROFILE_ID)
    ...
    def test_identity_tiktok_and_unclassable(self):
        ...
        self.assertIn("LA VAR", profile["known_alias"])
    ...
    def test_no_invented_or_forbidden_identity(self):
        ...
        for blob in blobs:
            ...
            self.assertIsNone(re.search(r"0\s*7\s*4\s*8\s*6\s*4\s*4\s*4\s*2\s*4", blob))
    ...
    def test_overlay_matches_census_exactly(self):
        ...
        self.assertIn("occupation:'Décorateur / Créateur TikTok'", OVERLAY)
        ...
        self.assertNotIn("name.includes('ange')", OVERLAY)
        self.assertNotIn("name.includes('boli')", OVERLAY)
    ...
    def test_loaders_and_persistence(self):
        self.assertIn("./profile-ange-boli.js?v=1.0", CONFIG)
        self.assertIn("./profile-ange-boli.js?v=1.0", SW)
