...
class OfficialLinksSearchJusteCrepinTests(unittest.TestCase):
    def test_search_uses_all_profiles_not_the_public_ranking(self):
        ...
        self.assertIn("pass50SearchSig", PROTECT)
        self.assertIn("function onOfficialLinksTab()", PROTECT)
        self.assertIn("requestAnimationFrame", PROTECT)
        self.assertNotIn("p50v9RenderLinks();", PROTECT.split("function applyOfficialLinksSearch", 1)[1].split("function ensureOfficialLinksSearch", 1)[0])
