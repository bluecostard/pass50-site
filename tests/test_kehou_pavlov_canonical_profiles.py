...
class KehouPavlovCanonicalProfileTests(unittest.TestCase):
    ...
    def test_sarra_messan_is_not_duplicated(self):
        variants = [
            ...
            if item.get("id") == "census-sarara-messan"
            or "messan" in str(item.get("name", "")).lower()
            or "messan" in str(item.get("known_alias", "")).lower()
        ]
        self.assertEqual(len(variants), 1)
        self.assertEqual(variants[0]["name"], "Sara")
        self.assertNotIn("sarara", variants[0]["name"].lower())
