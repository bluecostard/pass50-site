...
const PROFILE_ID='census-jiaan-wu';
const TIKTOK_URL='https://www.tiktok.com/@jiaaan.wu';
const INSTAGRAM_URL='https://www.instagram.com/jiaaan.wu/';
const FACEBOOK_URL='https://www.facebook.com/jiaan.wu.203389/';
const YOUTUBE_URL='https://www.youtube.com/@jiaaanwu';
let attempts=0;

function verifiedLink(message){
  return {status:'manual_verified',checkedAt:'2026-08-20T21:27:00.000Z',message};
}
...
function baseProfile(){
  return {
    ...
    initials:'JW',
    region:'CI',
    category:'Lifestyle / Divertissement',
    ...
    photoStatus:'missing',
    photoSource:'',
    photoNote:'Photo à valider depuis le compte TikTok @jiaaan.wu.',
    photoPosition:'50% 50%',
    ...
    linkChecks:{
      TikTok:verifiedLink('Compte TikTok @jiaaan.wu confirmé — créatrice ivoirienne (Jiaan Wu), recensé le 20 août 2026.'),
      Instagram:verifiedLink('Compte Instagram @jiaaan.wu recensé (Jiaan Wu).'),
      Facebook:verifiedLink('Profil Facebook jiaan.wu.203389 recensé (Jiaan Wu).'),
      YouTube:verifiedLink('Chaîne YouTube @jiaaanwu recensée.')
    },
    verifiedPass50:false,
    censusStatus:'Recensé confirmé',
    verificationPriority:'P0',
    source:{publisher:'Signalement PASS50 — profil TikTok Jiaan Wu',date:'2026-08-20',url:TIKTOK_URL},
    notes:'Jiaan Wu, créatrice ivoirienne (lifestyle). TikTok @jiaaan.wu · Instagram @jiaaan.wu · Facebook jiaan.wu.203389 · YouTube @jiaaanwu. Comptes reliés au Radar LIVE P0 PASS50. Date de recensement 20 août 2026, pas une date de naissance.'
  };
}
...
function applyProfile(){
  ...
  let profile=db.profiles.find(item=>{
    ...
    return item&&(
      item.id===PROFILE_ID
      ||name==='jiaan wu'
      ||name.includes('jiaan wu')
      ||handle==='@jiaaan.wu'
      ||handle==='jiaaan.wu'
      ||tiktok.includes('@jiaaan.wu')
      ||instagram.includes('jiaaan.wu')
    );
  });
  let changed=false;
