...
const PROFILE_ID='census-guyguy-le-grouilleur-de-bologne';
const TIKTOK_URL='https://www.tiktok.com/@guyguylegrouilleur07';
let attempts=0;

function verifiedLink(message){
  return {status:'manual_verified',checkedAt:'2026-08-24T17:00:00.000Z',message};
}
...
function baseProfile(){
  return {
    ...
    photoStatus:'missing',
    photoSource:'',
    photoNote:'Photo à valider depuis le compte TikTok @guyguylegrouilleur07.',
    photoPosition:'50% 50%',
    ...
    linkChecks:{
      TikTok:verifiedLink('Compte TikTok @guyguylegrouilleur07 confirmé — Guyguy le grouilleur de Bologne, recensé le 24 août 2026.')
    },
    verifiedPass50:false,
    censusStatus:'Recensé confirmé',
    verificationPriority:'P0',
    knownAlias:'Guyguy le grouilleur de Bologne / guyguylegrouilleurdebologne / @guyguylegrouilleur07',
    source:{publisher:'Signalement PASS50 — profil TikTok Guyguy le grouilleur de Bologne',date:'2026-08-24',url:TIKTOK_URL},
    notes:'Créateur TikTok. Compte officiel @guyguylegrouilleur07. Distinct de Le Grouilleur 3.0 (Iburaim). Identité civile et date de naissance non confirmées.'
  };
}
...
function applyProfile(){
  ...
  let profile=db.profiles.find(item=>{
    ...
    return item&&(
      item.id===PROFILE_ID
      ||name==='guyguy le grouilleur de bologne'
      ||compact==='guyguylegrouilleurdebologne'
      ||handle==='guyguylegrouilleur07'
      ||tiktok.includes('@guyguylegrouilleur07')
    );
  });
  let changed=false;
