...
Exception IONOS : un `unknown` (timeout API / embed) **après** un LIVE déjà confirmé ne réécrit plus `last_state` ; le flux reste public jusqu’à expiration de la fenêtre (8 min TikTok) ou un offline explicite. Coach Hamond Chic (`hamondchic` / `@coachhamond`) est en P0 seed.
