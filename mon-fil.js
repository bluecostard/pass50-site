...
(() => {
  ...
  async function loadNewsFor(profileId) {
    const query = new URLSearchParams({
      period: PERIODS[state.period] || '24h',
      profileId: String(profileId),
      newsLimit: String(NEWS_PER_PROFILE)
    });
    try {
  ...
  function renderFollowStrip() {
    ...
    strip.innerHTML = state.following.map(profileId => {
      ...
      return `<a class="follow-chip" href="./?profile=${encodeURIComponent(profile.id)}">
        ${avatarHtml(profile)}
        <div><strong>${esc(profile.name || 'Influenceur')}</strong><span class="rank">${rank ? `#${rank}` : 'À vérifier'} · Score ${Math.round(scoreFor(profile))}</span><span class="${change.className}">${esc(change.text)}</span></div>
      </a>`;
    }).join('');
  }
  ...
  function feedCard(item) {
    ...
    const original = /^https?:\/\//i.test(String(item.url || ''))
      ? `<a class="btn primary" href="${attr(item.url)}" target="_blank" rel="noopener">Voir le contenu original ↗</a>`
      : '';
    return `<article class="feed-card">
      <div class="feed-head">${avatarHtml(profile)}<div class="feed-person"><strong>${esc(profile.name || 'Influenceur')}</strong><span>${esc(profile.handle || '')}</span></div><div class="feed-position"><span class="${change.className}">${esc(change.text)}</span><br>${rank ? `#${rank}` : 'À vérifier'} · ${Math.round(scoreFor(profile))}/100</div></div>
      <div class="feed-media ${cover ? '' : 'no-image'}">${cover ? `<img src="${attr(cover)}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.parentElement.classList.add('no-image');this.remove()">` : '<span>📰</span>'}<div class="source-badge">${source}</div></div>
      <div class="feed-body"><h2>${esc(item.title || `Actualité récente de ${profile.name}`)}</h2><div class="feed-meta">${esc(meta)}</div><div class="feed-actions">${original}<a class="btn" href="./?profile=${encodeURIComponent(profile.id)}">Voir sa fiche et son classement</a></div></div>
    </article>`;
  }
  ...
  function normalizeLives(items) {
    ...
    return (Array.isArray(items) ? items : []).filter(item => {
      ...
      const key = [item.profileId || '', item.platform || '', String(item.url).replace(/\/+$/, '')]
  ...
  function renderRadarModal() {
    ...
    body.innerHTML = `<div class="radar-list">${state.liveStreams.map(live => {
      ...
      const details = [
        ...
        Number(live.viewers || 0) > 0 ? `${Number(live.viewers).toLocaleString('fr-FR')} spectateur(s)` : ''
      ].filter(Boolean).join(' · ');
  ...
  function installEvents() {
    ...
    $('#feedLiveModal')?.addEventListener('click', event => {
      if (event.target.id === 'feedLiveModal') closeRadar();
    });
  ...
  async function init() {
    ...
    window.PASS50_FOLLOW_FEED_PAGE = Object.freeze({
      contract: CONTRACT,
      maxFollowed: MAX_FOLLOWED,
      newsPerProfile: NEWS_PER_PROFILE
    });
  }
