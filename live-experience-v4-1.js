...
function buildShareCanvas(live,name){
  ...
  ctx.fillStyle='#eef1ec';ctx.fillRect(0,0,1080,1350);
  ctx.fillStyle='#b42318';ctx.fillRect(0,0,1080,18);
  ctx.fillStyle='#b7ff00';ctx.fillRect(64,56,22,22);
  ctx.fillStyle='#0b0f0b';ctx.font='1000 42px Inter,Arial,sans-serif';ctx.fillText('PASS50',100,76);
  ctx.fillStyle='#b42318';ctx.font='800 22px Inter,Arial,sans-serif';ctx.fillText('EN DIRECT',64,140);
  ctx.fillStyle='#ffffff';ctx.beginPath();ctx.arc(540,480,160,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#0b0f0b';ctx.lineWidth=4;ctx.stroke();
  ctx.fillStyle='#0b0f0b';ctx.textAlign='center';ctx.textBaseline='middle';ctx.font='1000 90px Inter,Arial,sans-serif';ctx.fillText(initials(name),540,485);
  ctx.textBaseline='alphabetic';ctx.fillStyle='#0b0f0b';ctx.font='1000 64px Inter,Arial,sans-serif';const nameLines=wrapLines(ctx,name,900);nameLines.forEach((line,index)=>ctx.fillText(line,540,720+index*74));
  const afterName=720+(nameLines.length-1)*74;ctx.fillStyle='#5c665c';ctx.font='600 30px Inter,Arial,sans-serif';ctx.fillText(`Sur ${String(live.platform||'le réseau')}`,540,afterName+70);
  ctx.fillStyle='#b7ff00';ctx.beginPath();ctx.roundRect(110,afterName+130,860,100,12);ctx.fill();ctx.fillStyle='#0b0f0b';ctx.font='1000 34px Inter,Arial,sans-serif';ctx.fillText('Regarder  →',540,afterName+194);
  ctx.fillStyle='#5c665c';ctx.font='600 24px Inter,Arial,sans-serif';ctx.fillText('pass50.store',540,1285);ctx.textAlign='start';return canvas;
}
...
function ensureShareModal(){
  ...
  .p50-live-share-box{width:min(430px,100%);border:1px solid #d5dbd2;border-radius:18px;background:#eef1ec;box-shadow:0 24px 80px rgba(0,0,0,.28);padding:12px}.p50-live-share-close{display:block;margin-left:auto;width:40px;height:40px;border-radius:50%;border:1px solid #cfd6cb;background:#fff;color:#0b0f0b;font-size:22px}
  .p50-live-share-card{min-height:430px;border-radius:14px;padding:24px;background:#eef1ec;border-top:8px solid #b42318;display:flex;flex-direction:column;align-items:center;text-align:center;color:#0b0f0b}.p50-live-share-brand{align-self:flex-start;display:flex;align-items:center;gap:10px;font-size:22px;font-weight:1000;letter-spacing:-.8px}.p50-live-share-brand::before{content:"";width:14px;height:14px;background:#b7ff00}.p50-live-share-pill{margin-top:22px;color:#b42318;font-size:13px;font-weight:900;letter-spacing:.4px}.p50-live-share-avatar{width:116px;height:116px;margin:26px 0 18px;border:2px solid #0b0f0b;border-radius:12px;display:grid;place-items:center;background:#dde3d8;font-size:40px;font-weight:1000}.p50-live-share-card h2{margin:0;font-size:28px;line-height:1.05;letter-spacing:-1px}.p50-live-share-network{margin-top:9px;color:#5c665c;font-weight:700}.p50-live-share-cta{width:100%;margin-top:auto;padding:15px;border-radius:10px;background:#b7ff00;color:#0b0f0b;font-weight:1000}.p50-live-share-actions{display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin-top:12px}.p50-live-share-actions button{min-height:52px;border:1px solid #cfd6cb;border-radius:12px;background:#fff;color:#0b0f0b;font-weight:950}.p50-live-share-actions button:first-child{background:#0b0f0b;color:#eef1ec;border-color:#0b0f0b}
...
function renderShareModal(live){
  ...
  modal.querySelector('.p50-live-share-avatar').textContent=initials(name);modal.querySelector('h2').textContent=name;modal.querySelector('.p50-live-share-network').textContent=`Sur ${String(live.platform||'le réseau')}`;modal.classList.add('show');modal.setAttribute('aria-hidden','false');
}
