...
(() => {
  const P50CI={period:'24h',data:null,loading:false,news:new Map(),lastRefresh:0};
  ...
  function renderTrends(){
    ...
    if(!items.length){grid.innerHTML='<div class="p50ci-empty">Aucun contenu ne progresse assez sur cette période pour entrer dans le Top 5.</div>';return;}
    grid.innerHTML=items.map(item=>{
      const cover=item.thumbnailUrl||fallbackCover(item.profileId),title=item.title||`Contenu récent de ${item.name}`;
      return `<a class="content-card p50ci-card" href="${attr(item.url)}" target="_blank" rel="noopener" data-content="${item.contentId}">
        ...
        <div class="p50ci-metrics"><span>Score ${Math.round(item.score)}/100</span><span class="${movementClass(item)}">${movement(item)}</span></div></div></a>`;
    }).join('');
  }

  async function refreshTrends(force=false){
    if(P50CI.loading)return;if(!force&&Date.now()-P50CI.lastRefresh<60000)return;
  ...
  function newsCard(item,index){
    const cover=item.thumbnailUrl||fallbackCover(item.profileId),extra=index>=3;
    return `<article class="p50ci-news-card ${extra?'is-extra':''}"><div class="p50ci-news-thumb">${cover?`<img src="${attr(cover)}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.remove()">`:'📰'}</div><div><h4>${esc(item.title||'Actualité récente')}</h4><div class="p50ci-news-meta"><span class="${item.official?'p50ci-official':''}">${item.official?'SOURCE OFFICIELLE':esc(item.sourceType)}</span> · ${esc(item.platform)} · ${esc(relative(item.publishedAt))}${item.trendBadge?` · ${esc(item.trendBadge)}`:''}</div></div><a class="btn small" href="${attr(item.url)}" target="_blank" rel="noopener">Voir ↗</a></article>`;
  }
  ...
  async function renderProfileNews(profileId){
    ...
    try{
      let data=P50CI.news.get(`${profileId}:${P50CI.period}`);if(!data){data=await fetchFeed(profileId);P50CI.news.set(`${profileId}:${P50CI.period}`,data);}
      if(!document.body.contains(shell))return;const items=data.news||[];
      shell.innerHTML=`<div class="p50ci-news-head"><div><div class="p50ci-news-title">📰 Actualité récente</div><div class="muted" style="font-size:11px">Publications officielles automatiques · informations externes validées</div></div>${items.length>3?'<button class="btn small" data-p50ci-expand>Voir toute l’actualité</button>':''}</div><div class="p50ci-news-list">${items.length?items.map(newsCard).join(''):'<div class="p50ci-empty">Aucune actualité récente validée pour cette fiche.</div>'}</div>`;
  ...
  function init(){injectStyles();installRenderHook();installProfileHook();installAdminHooks();bind();ensurePeriodControls();refreshTrends(true);setInterval(()=>refreshTrends(true),5*60*1000);}
